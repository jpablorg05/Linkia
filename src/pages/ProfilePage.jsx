...
import { Save, User, Building, Store, Clock, ShieldCheck, Image as ImageIcon, Tag, X, Truck } from 'lucide-react';
import ModuleHero from '../components/ModuleHero';
import api from '../services/api';

export default function ProfilePage() {
  const { user, setUser, sellerProfile, setSellerProfile, inventoryTags, setInventoryTags } = useAppContext();
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      // Guardar cambios en el usuario general del frontend
      ...
      alert(`Ocurrió un error al guardar el perfil: ${errorMessage}. ¿Reiniciaste el servidor backend (server/index.js)?`);
    }
  };
  ...
  return (
    <div className="animate-slide-up" style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
      ...
      <ModuleHero 
        title={user.role === 'buyer' ? 'Mi Perfil' : 'Configuración de Vitrina Pública'}
        eyebrow="Mi cuenta"
        iconName="profile"
        fullBleed={isImmersive}
      />

      <div style={{ width: '100%', maxWidth: '1200px', margin: '0 auto', padding: isImmersive ? '0 2rem 2rem' : '0' }}>
        <form onSubmit={handleSave} style={{ display: 'grid', gridTemplateColumns: user.role === 'seller' ? 'repeat(auto-fit, minmax(400px, 1fr))' : '1fr', gap: '2rem', alignItems: 'start' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
            ...
            {/* Tarjeta de Datos de Envío y Vehículo (Solo Compradores) */}
            {user.role === 'buyer' && (
              <div className="card-panel" style={{ padding: '2rem', background: 'var(--bg-panel)' }}>
                <h3 style={{ fontSize: '1.3rem', marginBottom: '1.5rem', borderBottom: '1px solid var(--border-color)', paddingBottom: '0.5rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  <Truck size={20} color="var(--primary)" /> Datos de Envío
                </h3>
                
                <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '1.5rem', marginBottom: '2rem' }}>
                  <div>
                    <label className="input-label">Dirección de Entrega Predeterminada</label>
                    <textarea name="address" value={formData.address || ''} onChange={handleUserChange} placeholder="Ej. Calle Principal, Edificio B, Apto 4, Caracas" className="input-field" style={{ minHeight: '80px', resize: 'vertical', marginBottom: 0 }} />
                  </div>
                </div>
            ...
            {user.role === 'seller' && (
              <div className="card-panel" style={{ padding: '2rem', background: 'var(--bg-panel)' }}>
                ...
                <div style={{ display: 'flex', gap: '2rem', marginBottom: '2rem', alignItems: 'center' }}>
                  <div style={{ width: 100, height: 100, borderRadius: '50%', background: 'var(--bg-main)', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '2px dashed var(--border-color)', color: 'var(--text-muted)', cursor: 'pointer', flexDirection: 'column', flexShrink: 0 }}>
                    <ImageIcon size={28} style={{ marginBottom: '0.5rem' }} />
                    <span style={{ fontSize: '0.65rem' }}>Subir Logo</span>
                  </div>
                  <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                    <p style={{ color: 'var(--text-muted)', fontSize: '0.85rem', marginBottom: '0.5rem' }}>Este logo y nombre se mostrará a los compradores cuando vean tus cotizaciones y perfil.</p>
